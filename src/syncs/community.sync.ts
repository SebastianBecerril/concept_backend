...
export const CreateCommunityRequest: Sync = (
  ...
  where: (frames) =>
    frames.query(
      UserAuthentication._getUserForSession,
      { sessionId: session },
      { user: creator },
    ),
  then: actions([
...
export const UpdateCommunityDetailsRequest: Sync = (
...
export const UpdateCommunityDetailsResponse: Sync = ({ request }) => ({
  ...
  then: actions([Requesting.respond, {
    request,
    status: "Community details updated successfully.",
  }]),
});

export const UpdateCommunityDetailsResponseError: Sync = (
  { request, error },
) => ({
  when: actions(
    [Requesting.request, { path: "/Community/updateCommunityDetails" }, {
...
export const AddMemberRequest: Sync = (
...
export const SetMemberRoleRequest: Sync = (
...
export const SetMemberRoleResponse: Sync = ({ request }) => ({
  ...
  then: actions([Requesting.respond, {
    request,
    status: "Member role updated successfully.",
  }]),
});
...
export const DeleteCommunityRequest: Sync = (
...
export const DeleteCommunityResponse: Sync = ({ request }) => ({
  ...
  then: actions([Requesting.respond, {
    request,
    status: "Community deleted successfully.",
  }]),
});
...
export const CascadeCommunityDeletionToPosts: Sync = (
  ...
  where: (frames) =>
    frames.query(CommunityBoard._getPostsByCommunity, { community }, { post }),
  then: actions(
...
export const GetAllMembershipsQuery: Sync = (
  { request, memberships },
) => ({
  when: actions(
  ...
  where: async (frames) => {
    ...
    const seed = frames[0] ?? {};
    return new Frames({ ...seed, [memberships]: records });
  },
  then: actions(
    [Requesting.respond, { request, memberships }],
  ),
});
