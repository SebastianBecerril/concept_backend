---
timestamp: 'Thu Oct 23 2025 20:29:42 GMT-0400 (Eastern Daylight Time)'
content_id: 913fa68a4cbc562effb7fa91cfa68882332e29f79a7650b12527aaa8cf27a91e
---
...
export default class CommunityConcept {
  ...
  async addMember({
    ...
    const userExists = await this.db.collection("UserAuthentication.users").findOne({ _id: user });
    if (!userExists) {
      return { error: "User does not exist." };
    }

    // Check if inviter exists
    const inviterExists = await this.db.collection("UserAuthentication.users").findOne({ _id: inviter });
    if (!inviterExists) {
    ...
    try {
      await this.memberships.insertOne(newMembership);
      // Update the community's memberships array
      await this.communities.updateOne(
        { _id: community },
